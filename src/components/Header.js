...
export default function Header() {
    ...
    return <header className='bg-white z-10 absolute top-0 left-0 right-0 h-20 flex justify-center align-center px-8 lg:px-40'>
        ...
        <nav className='hidden md:flex'>
            {navText.map( item => <a key={item} href="#" className='flex h-20 mx-2 lg:mx-3 my-auto no-underline text-grayish-blue align-middle border border-transparent border-4 hover:border-b-lime-green'><span className='my-auto'>{item}</span></a> )}
        </nav>
